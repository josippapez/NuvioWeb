  ...
  async loadManifestTrackDataForCurrentStream(playbackUrl = this.activePlaybackUrl) {
    ...
    try {
      ...
      for (const candidateUrl of urlCandidates) {
        ...
        const parsed = parseManifestTracks(fetchedManifest.text, fetchedManifest.finalUrl || candidateUrl);
        ...
        if (parsed.variants.length > 0) {
          ...
          try {
            ...
            const nestedParsed = parseManifestTracks(variantFetched.text, variantFetched.finalUrl || variant.uri);
            ...
            try {
              ...
              const nestedParsed = parseManifestTracks(variantFetchedNoHeaders.text, variantFetchedNoHeaders.finalUrl || variant.uri);
              if (nestedParsed.audioTracks.length || nestedParsed.subtitleTracks.length) {
                selectedParsed = nestedParsed;
